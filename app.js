...
// ---- LLEGIR localStorage (amb migració i validació) ----
let obtained = [];
...
try {
  ...
  if (Array.isArray(raw)) {
    ...
    // 1) Valors numèrics vàlids
    const numeric = raw.filter(v => Number.isInteger(v) && v >= 0 && v < stickers.length);

    // 2) Si hi ha strings (emoji antics), els convertim a índexs
...
// Guardem dades netes per assegurar coherència
